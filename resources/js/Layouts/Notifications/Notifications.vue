...
let checdiv: HTMLElement | null;

const emits = defineEmits(['toggleNotification'])

const notificationHandler = () => {
  if (!checdiv?.classList.contains("hidden")) {
...
onMounted(() => {
  ...
  checdiv = document.getElementById("chec-div");
  // Call notificationHandler on mount
  notificationHandler();
})

// Watch for changes in props.toggle
watch(() => props.toggle, (newVal) => {
    notificationHandler();
});
</script>
<template>
    <div class="w-full h-full bg-transparent top-0 overflow-y-auto overflow-x-hidden fixed sticky-0 z-50" id="chec-div">
            ...
            <div class="w-full absolute z-10 right-0 h-full overflow-x-hidden transform translate-x-0 transition ease-in-out duration-700" id="notification">
                <div class="2xl:w-4/12 bg-gray-50 h-screen overflow-y-auto p-8 absolute right-0">
                    <div class="flex items-center justify-between">
                        ...
                        <button role="button" aria-label="close modal" class="focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 rounded-md cursor-pointer" @click="notificationHandler">
                    ...
                    <ItemNotification v-for="notification in $page.props.auth?.notifications.data" :notification="notification" />
